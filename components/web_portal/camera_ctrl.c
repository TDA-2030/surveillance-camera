...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    int val = atoi(value);
    ESP_LOGI(TAG, "(%d):%s = %d", __LINE__, variable, val);
    sensor_t *s = esp_camera_sensor_get();
    int res = 0;
    ...
    if (!strcmp(variable, "framesize")) {
        ...
        // } else
        {
            ...
            int freq = s->xclk_freq_hz;
            esp_camera_deinit();
            vTaskDelay(pdMS_TO_TICKS(200));
            camera_init(freq, pixel_format, (framesize_t)val);
...
static const char *CAMERA_XCLK_NVS_KEY = "cam-xclk";
esp_err_t web_portal_camera_save_to_nvs(const char *key)
{
    ...
    if (ret == ESP_OK) {
        ...
        if (s != NULL) {
            ...
            ESP_LOGE(TAG, "can't get sensor failed");
        }
        nvs_close(handle);
        return ret;
    } else {
...
esp_err_t web_portal_camera_load_from_nvs(const char *key)
{
#if ESP_IDF_VERSION_MAJOR > 3
    nvs_handle_t handle;
#else
...
static esp_err_t save_param_handler(httpd_req_t *req)
{
    ESP_LOGI(TAG, "save parameter");

    esp_err_t ret = web_portal_camera_save_to_nvs("camera_param");
    if (ret) {
        return httpd_resp_send_500(req);
    }
...
void portal_camera_start(void)
{
    ...
    httpd_uri_t save_param_uri = {
        .uri = "/save_param",
        .method = HTTP_GET,
        .handler = save_param_handler,
        .user_ctx = NULL
    };
