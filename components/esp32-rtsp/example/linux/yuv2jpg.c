...
int yuv422_to_jpg(unsigned char*rData, unsigned long w, unsigned long h)
{
    unsigned char *in_Y = (unsigned char *)malloc(w * h); //
    unsigned char *in_U = (unsigned char *)malloc(w * h / 2); //
    unsigned char *in_V = (unsigned char *)malloc(w * h / 2); //
    unsigned char *pData = (unsigned char *)malloc(w * h); //

    unsigned long dwSize = 0;
    get_Y_U_V(rData, in_Y, in_U, in_V, w, h);

    YUV2Jpg(in_Y, in_U, in_V, w, h, 50, w, pData, &dwSize);
