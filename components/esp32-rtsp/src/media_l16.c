...
#define MAX_L16_PACKET_SIZE (RTP_MAX_PAYLOAD_SIZE - RTP_HEADER_SIZE - RTP_TCP_HEAD_SIZE)
...
media_stream_t *media_stream_l16_create(uint16_t sample_rate)
{
    ...
    memcpy(&stream->rtp_profile, rtp_profile_find(RTP_PAYLOAD_L16_CH1), sizeof(rtp_profile_t));
    stream->rtp_profile.frequency = sample_rate;  // Seems doc RFC3551 is incorrect, the colck frequency is not constant 44100
